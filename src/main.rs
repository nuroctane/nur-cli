...
async fn real_main() -> Result<()> {
    ...
    if cfg.provider == "opencode" {
        ...
        // Free models always snap to Zen, even if config still has the Go host
        // leftover from a previous metered model.
        if crate::providers::is_opencode_free_model(&cfg.model) {
            cfg.model = bare;
                ...
                && !crate::providers::is_opencode_go_model(&cfg.model)
            {
                ...
                let zen_hint = ml.contains("claude")
                    ...
                    || ml.contains("gemini")
                    || ml == "claude-sonnet-5"
                    || crate::providers::is_opencode_free_model(&ml);
                if zen_hint {
