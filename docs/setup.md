...
| Auth | Saves `NUR_API_KEY` if set in the environment (vendor keys such as `META_API_KEY` are read by their own provider, not saved) |
...
| Env-based auth | `NUR_API_KEY` -> saved under `~/.nur/auth.json` only |
