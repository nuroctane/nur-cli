...
| `typesafe.compaction.replace_summary` | bool | `true` | Manual `/compact` honors configured Jev without summarizing, even at zero reduction; automatic recovery also checks `min_reduction` |
| `typesafe.compaction.min_reduction` | float | `0.25` | Automatic recovery only: positive reduction required to skip summarization |
