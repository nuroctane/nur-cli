...
| [DeFiHackLabs](https://github.com/SunWeb3Sec/DeFiHackLabs) | Post-mortems as **regression tests**, not attack runbooks. TOC snapshot: `historical-smart-contract-vulns/references/incident-corpus.md` (858 names) |
| [Immunefi BugFixReviews](https://github.com/immunefi-team/Web3-Security-Library/blob/main/BugFixReviews/README.md) | Paid, patched cases |
| [sayan011 writeups list](https://github.com/sayan011/immunefi-bug-bounty-writeups-list) | Researcher + $ + link |
| [Immunefi scoreboard](https://immunefi.com/blog/research/the-ecosystem-vulnerability-scoreboard-6-years-of-defi-loss-data/) | Industry mix 2020-2026 |
| [SCV-List](https://github.com/sirhashalot/scv-list) | Living vuln catalog |
| Nur `hunting-x-linked-bounties` | Every public paid Immunefi/X row mapped to a playbook (load one slice) |
| Nur `historical-smart-contract-vulns` | Bounty + timeline + chain VM + **investigator-ops** (zachxbt/tayvano) library (load one slice) |
...
`@samczsun` `@pcaversaccio` `@pashovkrum` `@bytes032` `@cmichelio` `@0xRajeev` `@0xNazgul` `@0xJuancito` `@officer_cia` `@MuditGupta_` `@trustindistro` `@transmissions11` `@00xSEV` `@0xBarz` `@Defi_Nerd_sec` `@naeaexeth` `@anchabadze` `@PeterSRWeb3` `@QuillAudits_AI` `@0xvivekd` `@zachxbt` `@tayvano_`
