...
Load `sc-research` if you do not yet have scope. Companion checklist: `skills/security/SCA/auditing-foundry-smart-contract-security/references/vulnerability-checklist.md`.
...
| Wallet / Safe UI / Permit2 | simulation vs calldata, EIP-712 domain, leftover allowance (`investigator-ops.md`) |
...
Paid bounty analog: `hunting-x-linked-bounties` (one slice) then the Playbook column. Public hack analog: one slice of `historical-smart-contract-vulns`. Ingest T-ids: `ingest-class-map.md`. Do not dump either library.
