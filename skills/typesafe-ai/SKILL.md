...
license: MIT
description: >
  Build with TypeSafe: small units of AI intelligence you can use like
  programming primitives. Its System One models, including Jev, turn natural
  language and application state into typed judgments and probabilities that
  code can combine. Use when a step is a decision rather than writing - routing,
  ranking, relevance, verification, extraction, "does this need a human" - or
  when an LLM prompt-and-parse step could become a structured decision. Inside
  nur this layer is already wired into the harness (tool `typesafe`, `/typesafe`,
  `[typesafe]` config); this skill is for using it well, and for building with
  it in the user's own code.
---
...
flagship System One model. It understands natural language and returns typed answers
and probabilities rather than generating text or reasoning explanations. Code owns the
workflow; the model supplies programmable common sense where ordinary code needs
semantic understanding.

## Inside nur

nur already treats TypeSafe as a first-class harness layer, not a chat model:

- **Tool** `typesafe` - `ask` (batched, any mix of question types), `choice`,
  `noul`, `score`, `pick`, `rank`, `classify`, `risk`, `verify`, `status`.
- **Slash** `/typesafe` (alias `/jev`) - status, `on`/`off`, and `ask <state>`.
- **Config** `[typesafe]` in `~/.nur/config.toml` - thresholds, batching,
  `[typesafe.compaction]`, `[typesafe.tool_gate]`, `[typesafe.skills]`,
  `[typesafe.routing]`.
- **Key** - `TYPESAFE_API_KEY`, or `/auth` → `TypeSafe · Jev` (pinned at the top
  of the provider list), or `~/.nur/typesafe.key`.
- **In the loop** - tool-call gate, result judge, compaction, skill checks and
  model routing consult Jev when a key is present, and silently keep their
  previous behavior when it is not.

Runs keyless too: three open local engines (openJev-verdict-2.0, Bespoke-Nimble-9B,
Laya Core ML) serve the same contract behind one bridge on loopback - `nur jev start`
then `nur jev use`. A loopback endpoint needs no credential, so the whole layer
works offline on supported devices (`docs/jev-local.md`).

What that means for you as the agent: when a step is a decision - pick one of
these, how risky, is this relevant, did that work, does this need a person - ask
the `typesafe` tool instead of writing prose about it. For browser work,
`browser action=pick goal="..."` does the same for page elements: you get an
operation and an `@e` target chosen from the page's own indexed table. Batch every question you
might need into one `ask`, including speculative ones; independent questions in
one call run in parallel and cost one round trip.
...
1. **Candidates come from code.** Build the option list from the retriever, the
   trace, the registry - then let the model pick. Map the answer back onto your own
   value; a string that matches no candidate is not an option, it is a bug.
2. **Threshold on confidence, not on the answer.** `0.85+` may act; below the
   escalation floor (`0.5`) route to a bigger model or a human instead of obeying.

## References

- `references/nur-integration.md` - how nur wires this into the loop, with the
  exact config keys and the file each judgment lives in.
