...
The default server is `http://localhost:7236`. If that port is not active, read the `port` from `C:\Users\david\AppData\Roaming\tldraw\server.json`.
...
```bash
PORT=$(jq -r .port 'C:/Users/david/AppData/Roaming/tldraw/server.json'); TOKEN=$(jq -r .token 'C:/Users/david/AppData/Roaming/tldraw/server.json')
